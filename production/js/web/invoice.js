...
$(document).on('change', '#statusupdate', function() {
  if ($(this).val() == 'Pending Payment') {
          ...
          $('#tanggal_paymentupdate').attr('disabled',true);
         }
  });
...
$(".datepicker").datepicker({
    ...
    $("#status").change(function(){
         ...
         if ($(this).val() == 'Pending Payment') {
          $('#tanggal_payment').attr('disabled',false);
         }else{
          $('#tanggal_payment').attr('disabled',true);
         }
  ...
  $('#tambah-barang').click(function () {
    tambah();
   /* if ( $('#barang,#harga,#qty').parsley().validate()){
        tambah();
    }
    else
    {
  ...
  table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": site_url+"Master/runinvoice",
            "type": "POST",
             "data": function ( data ) {
...
function save(){
 ...
 $.ajax({
        context: this,
        url: site_url+"Master/addinvoice",
        type: "POST",
        ...
        success: function(data)
        {
            if(data.status) //if success close modal 
            {
                berhasil();
                $('#tambah')[0].reset();
                $('#transaksi-item').children().empty().remove();
                $('#total').text('');
                total = 0;
                $('#s2konsumen,#status').val('').trigger('change');
                $('#send').text('Submit'); 
...
function transaksiinvoice(nota){
      if(confirm('ingi menambahkan transaksi penlanggan di nota ini "'+nota+'"')){
    
        
        window.open(site_url+"Master/transaksiinvoice/"+nota, '_blank');
  } 
...
function history(nota){
      if(confirm('ingi melihat history hpp di nota ini "'+nota+'"')){
    
        
        window.open(site_url+"Master/historyhpp/"+nota, '_blank');
  } 
...
function hpsinvoice(id,ket){
      if(confirm('apakah ada yakin ingin menghapus data ini "'+ket+'"')){
    var url = site_url+"Master/hpsinvoice/";
  $.ajax({
        url : url+id,
        type: "POST",
        dataType: "JSON",
...
function detail(id,nota){
      if(confirm('ingi detail item nota ini "'+nota+'" ?')){
    
        
        window.open(site_url+"Master/detaillaporan/"+nota, '_blank');
  } 
...
function print(id,nota,type){
   var url = site_url+"Master/printinvoice/";
   var print;
  $.ajax({
        url : url+nota,
        dataType: "JSON",
  ...
  readyprint(print,type);

   
    return false;
}

function readyprint(data,type){
  ...
  items.push([
                {text:'Subtotal',colSpan: 4, alignment:'right',fillColor: '#dedede'},
                {},{},{},
                {text:data[0].total,fillColor: '#dedede'}
            ],
            [
                {text:'Kredit',colSpan: 4, alignment:'right',fillColor: '#dedede'},
                {},{},{},
                {text:data[0].totalkredit,fillColor: '#dedede'}
            ],
            [
                {text:'Total Kurang Bayar',colSpan: 4, alignment:'right',fillColor: '#dedede'},
                {},{},{},
                {text:data[0].kurangbayar,fillColor: '#dedede'}
            ]);
  transaksi.push([
                    {text:'Transaksi',colSpan:5,border: [false, false, false, false],style:'section'},
                    {},
                    {},
                    {},
                    {}
                    ...
                    {text:'Tanggal',fillColor: '#dedede'},
                    {text:'Keterangan',fillColor: '#dedede'},
                    {text:'Kredit',fillColor: '#dedede'},
                    {text:'Catatan',fillColor: '#dedede'}
                ]);
  i =0;
  data.transaksi.forEach(function (item) { 
           transaksi.push([i+1,item.tanggal,item.keterangan,formatRupiah(item.kredit.replace('.',','), "Rp. "),item.catatan]);
           i++;
  });
...
var dd = {
  ...
  content: [
        ...
        {text:'Invoice untuk :',fontSize: 9},
        {
            text:data[0].konsumen,
            bold: true,
            fontSize: 9,
        },
                ...
                body: [
                    ...
                    [{canvas: [ { type: 'line', x1: 0, y1: 0, x2: 80, y2: 0, lineWidth: 1 } ],
                     ...
                     [
                        ...
                        {},
                        {},
                        {text:'Manajer Keuangan',alignment:'center'},
                    ],
...
let promiseObject = pdfDocGenerator.getBase64((base64Data) => {
 ...
 promiseObject.then(function(result) {
  ...
  //window.location.href = 'data:image/jpg;base64,' + data;
...
{
   var url = site_url+"Master/getstatuspayment/";
    $('#formubah')[0].reset(); // reset form on modals
   //Ajax Load data from ajax
    var s2  = $('#statusupdate').select2({
    ...
    $.ajax({
        ...
        { 
          console.log(data.status);
            $('#id').val(data.id);
            $('#catatan_update').val(data.catatan);
            $('#tanggal_deadlineupdate').val(data.tanggal_deadline);
               $('#tanggal_paymentupdate').val(data.tanggal_payment);
          
           s2.append($('<option>').text(data.status));
           s2.val(data.status).trigger("change");
          
            $('#ubahstatus').modal('show'); // show bootstrap modal when complete loaded
...
function tambah(){
   ...
   var display = '<tr class="cart-value" id="'+key+'">' +
                  ...
                  '<td><input type="hidden" value="'+$('#qty').val()+'" name="trx['+key+'][qty]">'+ $('#qty').val() +'</td>' +
                  '<td><input type="hidden" value="'+$('#harga').val().split(".").join("")+'" name="trx['+key+'][harga]">'+ $('#harga').val() +'</td>' +
                  '<td id="total'+key+'"><input type="hidden" value="'+jumlah.split(".").join("")+'" name="trx['+key+'][penjualan]">'+  jumlah +'</td>' +
                  
                  '<td data-cart="'+key+'"><span class="btn btn-danger btn-sm transaksi-delete-item" data-cart="'+key+'">x</span></td>' +
                  '</tr>'; 
  total  = (+total) + (+jumlah.split('.').join('').replace(',','.'));
  $('#inputtotal').val(total);
  $("#transaksi-item").append(display);

  $('#total').text('Rp. '+total.toLocaleString('id'));
  $('#qty').val('');
  $('#harga').val('');
  $('#barang').val('');
...
function hapus(key){
     ...
     jumlah = $('#total'+key).text().split('.').join('').replace(',','.');
     total = (+total) -(+jumlah);
     $("#"+key).remove();
     $('#inputtotal').val(total);
     $('#total').text('Rp. '+total.toLocaleString('id'));
...
function check() {
$.ajax({
url: site_url+"Master/checktable/invoice",
 success: function(data)
        {
           ...
           json = JSON.parse(data);
            if(json.status) //if success close modal 
            {
              console.log('invoice: '+json.checksum);
                if(checksum == 0){
                  checksum = json.checksum;
                }
        ...
        error: function (jqXHR, textStatus, errorThrown)
        {
           info('error dalam check table invoice');
